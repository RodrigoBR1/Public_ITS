---
title: "Report 1.3: Empirical assesment of an *International Technological Subordination (ITS)*"
author: "Rodrigo BR"
date: "`r Sys.Date()`"
format: 
  docx:
...
2. **Hypothesis 2:** There must be a correlation between the indicators of the ITS, as they are all proxies for different forms of subordination in the global economy, and a correlation between these indicators and the economic complexity index (ECI) of a country, as a measure developed by @ECI_og2009 that seeks to measure the productive capabilities and / or *knowhow capital* of a country [^9]. According to the original authors of such an abstraction (that is, a number to represent and categorize countries according to their social accummulation of knowledge):
...
[^9]: For a critical review of the ECI, see @ECI_crit2018.

"[Knowhow capital accumulation] has not been a universal phenomenon. It has taken place in some parts of the world, but not in others. Where it has happened, it has underpinned an incredible increase in living standards. Where it has not, living standards resemble those of centuries past. The enormous income gaps between rich and poor nations are an expression of the vast differences in productive knowledge amassed by different nations." [@ECI_og2009]

Therefore, the ECI is expected to be at least mildly correlated with the compiled indicators of ITS, as countries at the top of the ECI ranking are expected to be self sufficient in the production of the capital goods analyzed in the ITS dataset, to the point of being net exporters of these goods or *knowhow capital*.
...
In this line, some trends are immediately evident. First, most countries in the world's regions are in a situation of dependence on imports of these capital goods (except in Europe, where the situation is a little more dynamic), insofar as most lines are concentrated on value 0 (equivalent to autarky in this context of analysis). Second, in the 30+ years of records, the tendency of the dominant countries is to lose dominance, not to previously subordinate countries, but to other dominante countries that are improving their position. A clear example of this is Asia, where Japan has seen its position as the strongest exporter in the economic sector weakened to give way to China as the most important exporter. Finally, if the -100 limit is considered as an indicator of countries that consolidate their position as net exporters, it is possible to say that in the Americas only three countries have reached this category: USA, Canada and Brazil, but only the first one has managed to maintain its position as a net exporter (although it has consistently worsened over time). Meanwhile, in Africa, no country has managed to be a net exporter of these products (even when the operations compiled consider the sale of used capital goods or those not produced in the country, which leads Kenya to have a record as a net exporter in 2004). Therefore, the real competition for global domination of the market for these capital goods is found over time between some European states (e.g. Germany, UK, Italy, Belgium or Spain), some Asian states (e.g. Japan, South Korea, India, Turkey or China) and the USA.
...
By plotting the patents count added (that is, the sum of patents in agriculture, mining and data processing technologies per country), two conclussions can be drawn. First, that the main global intellectual property owners [^8] in this sector of the economy have always beenthe USA and China, with a minor participation of South Korea, Japan, Russia (just recently), Canada and Australia. Second, that this field of global power dynamics doesn't have the same historical inertia as the TIDR, because just two countries are in very dominant possitions and the rest of the world can be considered as a "subordinate" in the global intellectual property system. Given the lack of historical inertia but clear pattern, this may be well considered as an emergent form of imperialism in the global economy.

[^8]: Let's remember, "family" implies patents that are in force in at least 2 countries at once (hence they are "related" as family) and allow for the inventor to protect the technology from reproduction without permission.
...
To understand the GFCF plot, the authors suggest to consider the 0 value as an indicator of a country that has a balanced relation between FDI inflows and outflows in relation to its capital formation. Therefore, an remebering that the FDIas GFCF data is measured as percentage instead of USD, the authors also suggest to interpret a value over 100 of net GFCF as a sign of a country that is receiving more than double the FDI it is sending out in relation to its capital formation. In the other hand, a value under -100 is a sign of a country that is sending more than double the FDI it is receiving in relation to its capital formation, and therefore is able to acces a position of financial domination in the global economy by investing in other territories.

First, it must be appointed that massive capital uses certain countries like bridges between economies to avoid taxes and other financial obligations according to international organizations (see @EuroParl2017 or @Oxfam2016), and this phenomena may explain why the data doesn't immediately shows the patterns previously registered (or even if it is capable of showing such patterns). For example, there are countries like Luxembourg or Cyprus that appear as huge net senders of FDI in relation to their capital formation, but to this point haven't appeared as imperialistic in the indicators nor the historical analysis (like, for example, USA or some european states). Therefore, the authors suggest that the international financial system may require a more detailed tracking of the FDI flows to identify the true territorial origin of the capital in movement; or to probe if this form of capital can even be localized at all, when transnational corporations are the ones making use of these *tax havens* that end up hiding the territorial origin of the capital.

Second, of the six continents, only Europe seems to have a relatively diverse set of countries involved in investing in other territories, with a strong presence of G20 nations but also smaller economies like Switzerland or Denmark. In the rest of the world, the data suggest that some of the main net investors in the global economy currently are the Netherlands, Japan, Canada and South Africa. This considers that some countries are pointed to possibly operate as *tax havens* (like Cyprus or Luxembourg, that rank 14 and 10 in the Corporate Tax Haven Index (CTHI) by the Tax Justice Network [@CTHI2024]) and therefore shouldn't be considered in the analysis. From the main net investors recognized, Japan seems to be the only steadily growing in time, while Canada and South Africa show volatile yet very positive trends.

## Second hypothesis testing - Work in progress

The authors suggest that the data gathered can be aggregated into an ITS index, which can be used to compare the relative positions of selected countries in the global economy (with a focus on Agriculture and Mining capital goods and patents ownership). For the purpose of testing *hypothesis 2*, the authors decided to composite an ITS synthetic indicator [^9] as a simple additive index without weights and normalized via Min-Max method (which involves *scaling* the original value of the variable to a 0 to 1 range, in order to identify the relative position of a country compared to the mean value of the variable), following the suggestions of @OECD2008. In other words, the ITS index seeks to represent the sum of power or sovereignty (as *national power*) capital of a given country in relation to others, so a country with a value closer to 0 should be considered as an indicator of low subordination (because of a surplus in capital goods to trade, patents to privde and / or money to invest), while a country with a value closer to 100 should be considered as an indicator of high subordination (because of an excesive dependency on technology imports, a low inventive activity and / or a high dependency on foreign investment).

[^9]: A synthetic indicator is a composite measure that seeks to represent a complex phenomenon through a simple formula, and it is often used in the social sciences to represent complex phenomena in a simple way. The OECD manual suggest to previously test the correlation between the variables to be aggregated, and to test the correlation between the synthetic indicator and the variables to be aggregated, to ensure the validity of the index. This process was done by the authors, and the results are presented in the Appendix C of this report, with its interpretation.

To build the index, the first step was to impute values in the variables of interest through the *mice* R package (developed by @mice2021), which makes *Multiple Imputation by Chained Equations* (hence the name of the package) of a defined data set, based on iterative modeling of each variable based on observed values of the other variables (this is expected to lead to more robust and reliable estimates of missing data than simple impute through mean value of two available observations). 

Then, the next step was to normalize the data using the following Min-Max formula:

$$
X_{\text{normalized}} = \frac{X - X_{\text{min}}}{X_{\text{max}} - X_{\text{min}}}
$$

Which to this point, produced a set of normalized and imputated variables along the set of original variables. The original dataset contained the following missing values structure:

```{r imputation_normalization_1}

# Load data

ITS_imp <- read_csv("data/ITS_imp-v1.csv")

#Table of missing values
miss_val <- colSums(is.na(ITS)) / nrow(ITS) * 100
miss_val <- tibble(
  Variable = names(miss_val),
  Missing = miss_val
)
...
Despite the high percentages of missing values in the data, when it was retrieved a lot of them were either related to the vast patent count difference between countries per years (so most countries countries get a lot of missing or NA values, while a few high inventive ones have a lot of observations through a long period of time), or to countries that have poor data availability in the UNCTAD and COMTRADE databases (often poor nations). Therefore, the imputation process was done selectively just to 
the variables of interest. Here follows density plots of the normalized and imputed data for the TIDR, patents count and net GFCF, where blue represents the original data and red represents the imputed data (purple is the overlap of both, which could be interpreted as a sign that indicates the final dataset shares a similar distribution with the original one).

```{r imputation_normalization_2}

# Load plots

knitr::include_graphics("plots/index-v1/IDIT_dens.png")
knitr::include_graphics("plots/index-v1/PATS_dens.png")
knitr::include_graphics("plots/index-v1/GFCF_dens.png")

```

Finally, the ITS index was constructed as the average of the normalized values of the TIDR, patents count and net GFCF, and then multiplied by 100 to get a percentage value. The final formula for the ITS index is as follows:

$$
\text{ITS index} = \left( \frac{\text{IDIT}_{\text{norm}} + \text{Net GFCF}_{\text{norm}} + \left(1 - \text{Patents count}_{\text{norm}}\right)}{3} \right) \times 100
$$

Two plots to show the distribution of the ITS index across the countries and years follow, and the analysis of each plot is written below it.

```{r ITS_plot1}

# Load plot
knitr::include_graphics("plots/index-v1/ITS_index_full.png")

```

On one hand, a view of the whole sample (199 countries and 34 years) is used to review a few key insights that the index shows and can be tracked to the previous descriptive analysis. In this plot, USA and China (the two main dominant actors throughout the data, specially in patents) clearly have the two lowest ITS index values across time, with the latter achieving a stronger position during the last decade.

```{r ITS_plot2}

# Load plot
knitr::include_graphics("plots/index-v1/ITS_index_faceted.png")

```

With the side-to-side view of ITS distribution per continent, its clear that the synthetic indicator displays the general patterns already identified in the TIDR, patents count and net GFCF plots, with a little more softness in the distribution of the values. For example, Americas, Asia and specially Europe show more countries that are able to stay between the 0.6 - 0.5 range (which is in this context sets countries in a relatively less subordinated position in the global economy), while Africa and Oceania only have countries with values above 0.6 (that is, in a relatively more subordinated position in the global economy). It could be said that only Americas and Asia are being able to produce not-subordinated countries in the global economy in the sectors analyzed, while Europe is in a position slowly but steadily weakening its position as a dominant actor in the global economy.

# A subsample of latin american countries as case study for the ITS indicators

A sample of countries within Latin America is selected for an in depth analysis of the ITS indicators. The countries selected are Argentina, Brazil, Bolivia, Chile, Colombia, Mexico, Paraguay and Peru, and the analysis will be focused on describing the historical trends rather than proving subordination per se. The authors suggest this analysis as a way to show and evaluate the potential of the dataset and its construction logic for further research in the field of the ITS.
...
# Load plot
knitr::include_graphics("plots/plots-v2/GFCF_plot_LAS.png")

```

First, it could be stated that all subsampled Latin American economies are net receivers of FDI, given that all of them (except Brazil in 1990) have a positive net GFCF value; but the size of this ratio (that is, how much more FDI as percentage of local Capital is being received rather than sent by the country) changes over time and across the subregion. In one hand, Chile and Bolivia share that in the data they have always received not less than a 100% more than their are sending out as stock of Gross Fixed Capital Formation[^10], with a peak in foregin investment between approximately 1997 and 2008 slighly shared with Argentina and Peru. In the other hand, countries like Brazil, Paraguay, Mexico, and specially Peru and Colombia, show a deepening of their financial subordination in the global economy accross time, if one is to consider that an upwards trajectory in the net GFCF ratio is a sign of a country that is receiving more and more FDI in relation to its outflows of FDI. Therefore, it could be said that the data suggests that a financial subordination of the Latin American subsample as a dynamic processs, with some countries like Chile and Bolivia having a clear position as net receivers of FDI, and others like Brazil, Colombia and Peru deepening their position as net receivers of FDI.

[^10]: FDI as net stock of GFCF can be a complex measure to use, but the authors suggest its number highlights two key factors in the global economy: first, the capacity of a country to attract capital from other territories (that is, allows to track stock of FDI in a country) in contrast to its capacity to invest abroad (which is represented by the sign that the result of the ratio gives); and second, the idea that FDI becomes problematic when it is at the base of the capital formation of a country, and therefore gives the investor a certain level of control over the local economy. 

```{r PATS_LAS}

# Load plot
knitr::include_graphics("plots/plots-v2/pats_plot_LAS.png")

```

Just as in the global economy data, the Latin American subsample data shows a strata of countries that produce intellectual property in the areas of interest and other that simply don't. In this subsample, Brazil and Mexico appear as the only countries that register at least 1 year with 1000 or more internationalized patents in force, with a clear dominance of Brazil in the region; while the rest of the countries in the subsample show a very low level of inventive activity in the sectors of interest.

## TIDR among selected Latin American countries: the Brazilian case

```{r TIDR_LAS}

# Load plot
knitr::include_graphics("plots/plots-v2/TIDR_plot_LAS.png")

```

 Brazil is the only country in the selected sample that shows a pattern of lowering its dependency from the USA through time, while increasing its dependency from China. It is, also the only country that has shown clear signs of belonging to the stronger economies in the world (i.e. it the highest invention exporter in the region, the only net exporter in the subsample and the country with the most complex and financially powerful economy among the subsample), and therefore said change could be intepreted as a sign of a shift in the global economy towards China as the main exporter of technified capital goods, even among not-so-subordinated countries or *middle economies* like Brazil.

## The ITS index for the Latin American subsample

Analysis pending, visualizations below.
```{r ITS_LAS}

# Load plots
knitr::include_graphics("plots/index-v1/ITS_LAS.png")
knitr::include_graphics("plots/index-v1/ITS_ECI_LAS.png")
...
# Appendix C: Correlation matrix between the ITS indicators and the ITS index

The following correlation matrix was constructed to test the validity of the ITS index as a synthetic indicator of the subordination of a country. In each box, the correlation coefficient is presented, with the following interpretation: a value closer to 1 indicates a strong positive correlation between the two variables, a value closer to -1 indicates a strong negative correlation between the two variables, and a value closer to 0 indicates no correlation between the two variables. The interpretation of the correlation matrix is written below it.

```{r appendix_c}

# Load corplot
knitr::include_graphics("plots/index-v1/corplot_ITS.png")
